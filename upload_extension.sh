...
DevelopVersionString=`grep -E 'version.develop.*=' DCAModule_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' DCAModule_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' DCAModule_Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' DCAModule_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" DCAModule_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' DCAModule_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" DCAModule_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' DCAModule_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" DCAModule_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' DCAModule_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' DCAModule_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" DCAModule_Extension.podspec
...
pod repo push ${Repo} DCAModule_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
